...
print('overlay1_descs = %d' % (len(ol_portrait)))
for i in range(len(ol_portrait)):
    desc = ol_portrait[i]
    print('overlay1_desc%d = "%s,%f,%f,%s,%f,%f"' % (i, desc.name, desc.x / w, desc.y / h, desc.shape, desc.rx / w, desc.ry / h))
    if desc.img is not None:
        print('overlay1_desc%d_overlay = %s' % (i, desc.img))
    if desc.target is not None:
        print('overlay1_desc%d_next_target = "%s"' % (i, desc.target))
